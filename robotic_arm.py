...
class Robot(object):
  def __init__(self, Q, R, Qf):
    ...
    # TODO：加速度待定
    self.xmax   = np.array([ 2.6179,  3.1415,  5,  
                             2.9670,  3.1415,  5,  
                             0.0,  3.1415,  5, 
                             1.5184, 3.1415,  5,  
                             1.3439,  3.1415,  5, 
                             2.7925,  3.1415,  5])
    #Limits are on ----------joints,  velocities, accelerations-----------------
    self.xmin   = np.array([ -2.6179, -3.1415,  -5,  
                             0.0,  -3.1415,  -5,  
                             -2.8797,  -3.1415,  -5, 
                             -1.5184  -3.1415,  -5,  
                             -1.3439,  -3.1415,  -5, 
  ...
  def add_obs_constraint(self, prog, x, q_avoid_list, N, side = 4, center = 0.75):
        ...
        for k in range(len(q_avoid_list)):
            for horizon in range(N):
                q_in_x = np.array([x[horizon][0], x[horizon][3], x[horizon][6], x[horizon][9], x[horizon][12], x[horizon][15]])
  ...
  def continuous_time_full_dynamics(self, x, u):
    ...
    #Expand for 6D
    Acontinuous = block_diag(Ac, Ac, Ac, Ac, Ac, Ac)
    Bcontinuous = block_diag(Bc, Bc, Bc, Bc, Bc, Bc)
